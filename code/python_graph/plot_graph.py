...
def get_data_from_device(com_port, rl, start_freq_index, stop_freq_index):
    ...
    if(com_port == None):
        ...
        for port in sorted(ports):
            if((port.vid == 0x0483)and(port.pid == 0x5740)): #STM32 Virtual COM Port
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.data_line = self.plot_graph.plot(x=self.x, y=self.y, pen=self.pen, symbol='o',symbolSize=3)
    ...
    def update_plot_data(self):
        ...
        if not self.btn.isChecked():
            self.y = get_data_from_device(self.com_port, self.rl, self.index_start, self.index_stop)
